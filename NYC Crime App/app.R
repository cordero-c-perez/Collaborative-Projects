...
ui <-  navbarPage("NYC Crime App", theme = shinytheme("flatly"),
  ...
  tabPanel("Information & Precinct Map",
    ...
    sidebarPanel( width = 4,
      ...
      textInput(inputId = "enteredAddress", label = "Enter Address to Pin a Location",
                placeholder = "Ex. 20 W 34th St, New York, NY 10001"),
      strong(span("Note:", style = "color:blue")),
      p("The map may take a few seconds to load when an address is entered.")
...
server <- shinyServer(function(input, output) {
  ...
  output$cpleth <- renderLeaflet({
    ...
    if (input$enteredAddress == ""){
    
      leaflet(my_spdf) %>% 
        addPolygons(stroke = FALSE, smoothFactor = .3, fillOpacity = .2, 
